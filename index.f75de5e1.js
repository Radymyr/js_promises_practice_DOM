...
const showNotification = (someData)=>{
    ...
    if (someData.success) div.classList.add("success");
    if (!someData.success) div.classList.add("error");
...
firstPromise.then((successData)=>showNotification(successData)).catch((errorData)=>showNotification(errorData));
...
const thirdPromise = ()=>{
    ...
    Promise.all([
        promise1,
        promise2
    ]).then((successData)=>showNotification(successData[0]));
};
thirdPromise();
